...
2. **ERP** (`/erp`) - ERP module with collapsible sidebar sub-pages: Finance, Procurement, Inventory, Payments. Includes Qashio and Tagway external links. Routes: `/erp/finance`, `/erp/procurement`, `/erp/inventory`, `/erp/payments`.
   - **Requisitions List** (`/erp/procurement/requisitions`) - Table view with search, workflow stage filter (Submitted/Pending Line Manager/Pending Purchasing Review/Pending Budget Owner/Pending Final Approval/Ready for Purchase/PO Created/Rejected), click-through to detail, per-row PDF download
   ...
   - **Approval Workflow**: Multi-step workflow engine (Submitted → Pending Line Manager → Pending Purchasing Review → Pending Budget Owner → [amount-based: ≤5000 AED → Ready for Purchase; >5000 → Pending Final Approval] → Ready for Purchase → PO Created). Rejection at any stage marks the requisition as Rejected. Workflow logic in `server/workflow.ts` with pluggable router interface for approver assignment. Data model: `requisition_approval_steps` table tracks each step (stage, assignedTo, assignedToGroup, decision, comments, timestamps). The Purchasing Review stage uses group-based assignment (cost center 118001003) — any employee in the Procurement department can take action on the requisition.
