...
2. **ERP** (`/erp`) - ERP module with collapsible sidebar sub-pages: Finance, Procurement, Inventory, Payments. Includes Qashio and Tagway external links. Routes: `/erp/finance`, `/erp/procurement`, `/erp/inventory`, `/erp/payments`.
   - **Requisitions List** (`/erp/procurement/requisitions`) - Table view with search, workflow stage filter (Submitted/Pending Line Manager/Pending Purchasing Review/Pending Budget Owner/Pending Finance Review/Ready for Purchase/PO Created/Rejected), click-through to detail, per-row PDF download
   - **Requisition ARF** (`/erp/procurement/requisitions/new`) - Approval Request Form with 6 sections: request info, description, justification, budget details (AED cost, budgeted yes/no, vendor), file uploads (JPG/PNG/PDF), timeline. On submission, automatically initializes a multi-step approval workflow
   - **Requisition Detail** (`/erp/procurement/requisitions/:id`) - Full detail view with approval timeline/audit trail, approve/reject action buttons (for assigned approvers/admins), attachment upload during Purchasing Review stage, comments, summary cards
   - **My Approvals** (`/my-approvals`) - Queue of requisitions pending the current user's approval action, with requisition details and review links. Accessible from sidebar under "Workflow" section
   - **Vendor Quotations** - Procurement approvers can add multiple vendor quotations (vendor name, file upload, recommended flag, comments) during Purchasing Review stage. Only one quotation can be recommended at a time. Quotations visible to subsequent approvers. Data model: `requisition_quotations` table.
   - **Approval Workflow**: Multi-step workflow engine (Submitted → Pending Line Manager → Pending Purchasing Review → Pending Budget Owner → Pending Finance Review → Ready for Purchase → PO Created). After Budget Owner approval, the request always moves to a mandatory Finance Review step where any user with the "finance" role can flag budget availability (Available / Not Available). Both outcomes move the request to "Ready for Purchase". Rejection at any stage marks the requisition as Rejected. Workflow logic in `server/workflow.ts` with pluggable router interface for approver assignment. Data model: `requisition_approval_steps` table tracks each step (stage, assignedTo, assignedToGroup, decision, comments, budgetFlag, timestamps). The Purchasing Review stage uses group-based assignment (cost center 118001003). The Finance Review stage uses role-based assignment (assignedToGroup="finance") — any user with the "finance" role can act. The approval_matrix table and admin page remain in the codebase but are no longer used by the workflow.
...
13. **Azure Tables** (`/azure-tables`) - NetSuite data tables synced from Azure. 17 tables: Account Lines, Accounts, Classes, Customers, Deleted Transactions, Departments, Employees, Entities, Horses, Items, Locations, Subclasses, Subsidiaries, Transactions, Transactions Test, Vendors, Transaction Lines. Card grid with record counts, click-through to paginated data table with search. API: `/api/azure-tables/summary` (counts), `/api/azure-tables/:tableName` (paginated rows). Schema tables prefixed `ns_` in PostgreSQL.
...
- **Tables with FK constraints**: `password_reset_tokens`, `tickets`, `ticket_comments`, `ticket_attachments`, `audit_logs`, `import_logs`, `spaces`, `project_groups`, `projects`, `project_assignments`, `project_comments`, `sm_billing_elements`, `sm_livery_agreements`, `sm_invoices`, `sm_invoice_lines`
