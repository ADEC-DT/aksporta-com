...
function SubSidebar({
  ...
  return (
    <div
      ...
      data-testid="sidebar-sm-hub"
    >
      {!collapsed && (
        <div className="px-4 pt-4 pb-2 border-b border-border">
          <div className="text-sm font-bold text-foreground">Stable Master</div>
      ...
      <div className="flex items-center justify-end px-1 py-1 border-b border-border">
        <Button variant="ghost" size="icon" onClick={onToggle} data-testid="button-toggle-smh-sidebar">
              ...
              {section.items.map((item) => {
                  ...
                  return (
                    <div key={item.id}>
                      <Button
                        ...
                        data-testid={`button-smh-expand-${item.id}`}
                      >
                        ...
                        item.children.map((child) => {
                          ...
                          return (
                            <Button
                              ...
                              data-testid={`button-smh-nav-${child.id}`}
                            >
                ...
                return (
                  <Button
                    ...
                    data-testid={`button-smh-nav-${item.id}`}
                  >
                    {Icon && <Icon className="h-3.5 w-3.5" />}
...
function SchedulePage() {
  return (
    <Card>
      ...
      <CardContent>
        <p className="text-muted-foreground" data-testid="text-smh-schedule-placeholder">Scheduled activities will be built later.</p>
      </CardContent>
    </Card>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-stables-title">Stables</h2>
        <Button onClick={() => { resetForm(); setDialogOpen(true); }} data-testid="button-smh-add-stable"><Plus className="h-4 w-4 mr-1" /> Add Stable</Button>
      </div>
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {stables.map((s) => {
            ...
            return (
              <Card key={s.id} data-testid={`card-smh-stable-${s.id}`}>
                <CardHeader className="flex flex-row items-center justify-between gap-4 py-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => openEdit(s)} data-testid={`button-smh-edit-stable-${s.id}`}><Pencil className="h-3.5 w-3.5" /></Button>
                    <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(s.id)} data-testid={`button-smh-delete-stable-${s.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                </CardHeader>
              </Card>
            );
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Name *</Label><Input value={name} onChange={(e) => setName(e.target.value)} data-testid="input-smh-stable-name" /></div>
            <div className="space-y-1"><Label>Notes</Label><Textarea value={notes} onChange={(e) => setNotes(e.target.value)} data-testid="input-smh-stable-notes" /></div>
            <div className="flex items-center gap-2"><Label>Active</Label><Switch checked={isActive} onCheckedChange={setIsActive} data-testid="switch-smh-stable-active" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => { if (!name.trim()) { toast({ title: "Name required", variant: "destructive" }); return; } mutation.mutate({ name, notes: notes || null, isActive }); }} disabled={mutation.isPending} data-testid="button-smh-submit-stable">{mutation.isPending ? "Saving..." : "Save"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function BoxesPage() {
  ...
  const [genType, setGenType] = useState("STALL");
  ...
  const filtered = useMemo(() => {
    return boxes.filter((b) => {
      ...
      if (filterType && filterType !== "all" && b.boxType !== filterType) return false;
  ...
  const generateMutation = useMutation({
    mutationFn: async (data: Record<string, unknown>) => {
      ...
      for (let i = 1; i <= count; i++) {
        promises.push(apiRequest("POST", "/api/sm/boxes", {
          name: `${data.prefix}${String(i).padStart(2, "0")}`,
  ...
  const handleGenerate = () => {
    if (!genStableId || !genPrefix.trim()) {
      toast({ title: "Stable and prefix required", variant: "destructive" });
      return;
    }
    const count = parseInt(genCount, 10);
    if (!count || count < 1 || count > 50 || isNaN(count)) {
      toast({ title: "Count must be between 1 and 50", variant: "destructive" });
      return;
    }
    generateMutation.mutate({ stableId: genStableId, prefix: genPrefix, count: String(count), boxType: genType });
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-boxes-title">Boxes</h2>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => { setGenStableId(""); setGenPrefix(""); setGenCount("5"); setGenType("STALL"); setGenDialogOpen(true); }} data-testid="button-smh-generate-boxes">Generate Boxes</Button>
          <Button onClick={() => { resetForm(); setDialogOpen(true); }} data-testid="button-smh-add-box"><Plus className="h-4 w-4 mr-1" /> Add Box</Button>
        </div>
      </div>
      <div className="flex items-center gap-2 flex-wrap mb-4">
        <div className="relative"><Search className="absolute left-2 top-2.5 h-3.5 w-3.5 text-muted-foreground" /><Input placeholder="Search boxes..." value={searchQ} onChange={(e) => setSearchQ(e.target.value)} className="pl-7 h-9 w-[180px]" data-testid="input-smh-search-boxes" /></div>
        <Select value={filterStable} onValueChange={setFilterStable}>
          <SelectTrigger className="w-[150px] h-9" data-testid="select-smh-filter-stable"><SelectValue placeholder="All stables" /></SelectTrigger>
        ...
        <Select value={filterType} onValueChange={setFilterType}>
          <SelectTrigger className="w-[130px] h-9" data-testid="select-smh-filter-type"><SelectValue placeholder="All types" /></SelectTrigger>
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          <SelectTrigger className="w-[140px] h-9" data-testid="select-smh-filter-status"><SelectValue placeholder="All statuses" /></SelectTrigger>
        ...
        {(filterStable || filterType || filterStatus || searchQ) && (
          <Button variant="ghost" size="sm" onClick={() => { setFilterStable(""); setFilterType(""); setFilterStatus(""); setSearchQ(""); }} data-testid="button-smh-clear-filters">Clear</Button>
        )}
      </div>
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {filtered.map((b) => (
                <tr key={b.id} className="border-b border-border/50" data-testid={`row-smh-box-${b.id}`}>
                  ...
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(b)} data-testid={`button-smh-edit-box-${b.id}`}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(b.id)} data-testid={`button-smh-delete-box-${b.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Name *</Label><Input value={name} onChange={(e) => setName(e.target.value)} data-testid="input-smh-box-name" /></div>
            <div className="space-y-1">
              <Label>Stable *</Label>
              <Select value={stableId} onValueChange={setStableId}>
                <SelectTrigger data-testid="select-smh-box-stable"><SelectValue placeholder="Select stable" /></SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              <Select value={boxType} onValueChange={setBoxType}>
                <SelectTrigger data-testid="select-smh-box-type"><SelectValue /></SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              <Select value={status} onValueChange={setStatus}>
                <SelectTrigger data-testid="select-smh-box-status"><SelectValue /></SelectTrigger>
            ...
            <div className="space-y-1"><Label>Notes</Label><Textarea value={boxNotes} onChange={(e) => setBoxNotes(e.target.value)} data-testid="input-smh-box-notes" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => { if (!name.trim() || !stableId) { toast({ title: "Name and stable required", variant: "destructive" }); return; } mutation.mutate({ name, stableId, boxType, status, notes: boxNotes || null }); }} disabled={mutation.isPending} data-testid="button-smh-submit-box">{mutation.isPending ? "Saving..." : "Save"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={genDialogOpen} onOpenChange={setGenDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Generate Boxes</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
              <Label>Stable *</Label>
              <Select value={genStableId} onValueChange={setGenStableId}>
                <SelectTrigger data-testid="select-smh-gen-stable"><SelectValue placeholder="Select stable" /></SelectTrigger>
                <SelectContent>{stables.map((s) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-1"><Label>Name Prefix *</Label><Input placeholder="e.g. Box-A" value={genPrefix} onChange={(e) => setGenPrefix(e.target.value)} data-testid="input-smh-gen-prefix" /></div>
            <div className="space-y-1"><Label>Count</Label><Input type="number" min="1" max="50" value={genCount} onChange={(e) => setGenCount(e.target.value)} data-testid="input-smh-gen-count" /></div>
            <div className="space-y-1">
              <Label>Box Type</Label>
              <Select value={genType} onValueChange={setGenType}>
                <SelectTrigger data-testid="select-smh-gen-type"><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="STALL">Stall</SelectItem><SelectItem value="PADDOCK">Paddock</SelectItem><SelectItem value="STORAGE">Storage</SelectItem></SelectContent>
              </Select>
            </div>
            {genPrefix && genCount && (
              <p className="text-xs text-muted-foreground">Preview: {genPrefix}01, {genPrefix}02, ... {genPrefix}{String(Math.min(Number(genCount), 50)).padStart(2, "0")}</p>
            )}
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setGenDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleGenerate} disabled={generateMutation.isPending} data-testid="button-smh-submit-generate">{generateMutation.isPending ? "Generating..." : "Generate"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function HorsesPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-horses-title">Horses</h2>
        <div className="flex items-center gap-2">
          <input type="file" ref={fileInputRef} accept=".xlsx,.xls,.csv" className="hidden" onChange={handleFileUpload} data-testid="input-smh-horse-import-file" />
          <Button variant="outline" onClick={() => { resetImportDialog(); setImportDialogOpen(true); }} disabled={previewMutation.isPending} data-testid="button-smh-import-horses">
            {previewMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}
            Import Excel
          </Button>
          <Button onClick={() => { resetForm(); setDialogOpen(true); }} data-testid="button-smh-add-horse"><Plus className="h-4 w-4 mr-1" /> Add Horse</Button>
        </div>
      </div>
      <div className="flex items-center gap-2 flex-wrap mb-4">
        <div className="relative">
          <Search className="absolute left-2 top-2.5 h-3.5 w-3.5 text-muted-foreground" />
          <Input placeholder="Search horses..." value={searchQ} onChange={(e) => { setSearchQ(e.target.value); setCurrentPage(1); }} className="pl-7 h-9 w-[200px]" data-testid="input-smh-search-horses" />
        </div>
      </div>
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {paginatedHorses.map((h) => (
                <tr key={h.id} className="border-b border-border/50" data-testid={`row-smh-horse-${h.id}`}>
                  ...
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(h)} data-testid={`button-smh-edit-horse-${h.id}`}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(h.id)} data-testid={`button-smh-delete-horse-${h.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between border-t pt-4 mt-4">
              <p className="text-sm text-muted-foreground" data-testid="text-smh-horses-showing">
                Showing {((currentPage - 1) * pageSize) + 1}–{Math.min(currentPage * pageSize, filtered.length)} of {filtered.length}
              </p>
              <div className="flex items-center gap-1">
                <Button variant="outline" size="sm" disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} data-testid="button-smh-horses-prev-page">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, idx) =>
                    p === "..." ? (
                      ...
                      <Button key={p} variant={currentPage === p ? "default" : "outline"} size="sm" className="min-w-[36px]" onClick={() => setCurrentPage(p as number)} data-testid={`button-smh-horses-page-${p}`}>
                        {p}
                      </Button>
                    )
                  )}
                <Button variant="outline" size="sm" disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)} data-testid="button-smh-horses-next-page">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Name *</Label><Input value={name} onChange={(e) => setName(e.target.value)} data-testid="input-smh-horse-name" /></div>
            <div className="space-y-1"><Label>Color</Label><Input value={color} onChange={(e) => setColor(e.target.value)} data-testid="input-smh-horse-color" /></div>
            <div className="space-y-1">
              <Label>Sex</Label>
              <Select value={sex} onValueChange={setSex}>
                <SelectTrigger data-testid="select-smh-horse-sex"><SelectValue placeholder="Select sex" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1"><Label>Date of Birth</Label><Input type="date" value={dob} onChange={(e) => setDob(e.target.value)} data-testid="input-smh-horse-dob" /></div>
            <div className="space-y-1"><Label>Remarks</Label><Textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} data-testid="input-smh-horse-remarks" /></div>
            <div className="space-y-1">
              <Label>Status</Label>
              <Select value={status} onValueChange={setStatus}>
                <SelectTrigger data-testid="select-smh-horse-status"><SelectValue /></SelectTrigger>
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => { if (!name.trim()) { toast({ title: "Name required", variant: "destructive" }); return; } mutation.mutate({ name, color: color || null, sex: sex || null, dob: dob || null, remarks: remarks || null, status }); }} disabled={mutation.isPending} data-testid="button-smh-submit-horse">{mutation.isPending ? "Saving..." : "Save"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={importDialogOpen} onOpenChange={(open) => { setImportDialogOpen(open); if (!open) resetImportDialog(); }}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          {importStep === "upload" && (
            <div className="space-y-4 py-4">
              <div className="border-2 border-dashed rounded-lg p-8 text-center space-y-3">
                ...
                <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={previewMutation.isPending} data-testid="button-smh-select-horse-file">
              ...
              <div className="grid gap-3">
                {HORSE_FIELDS.map((field) => (
                  <div key={field.key} className="flex items-center gap-3">
                    ...
                    >
                      <SelectTrigger data-testid={`select-smh-horse-map-${field.key}`} className="w-[220px] overflow-hidden">
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-3 gap-2 text-sm">
                <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-green-700 dark:text-green-400" data-testid="text-smh-import-imported">{importResult.imported}</p>
                  <p className="text-xs text-muted-foreground">Imported</p>
                </div>
                <div className="text-center p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-yellow-700 dark:text-yellow-400" data-testid="text-smh-import-skipped">{importResult.skipped}</p>
                  <p className="text-xs text-muted-foreground">Skipped</p>
                </div>
                <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-blue-700 dark:text-blue-400" data-testid="text-smh-import-total">{importResult.totalRows}</p>
          ...
          <DialogFooter>
            {importStep === "mapping" && (
              <>
                <Button variant="outline" onClick={() => setImportStep("upload")} data-testid="button-smh-horse-import-back">Back</Button>
                <Button onClick={handleImportSubmit} disabled={!columnMapping.name || importMutation.isPending} data-testid="button-smh-horse-start-import">
                  {importMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Importing...</> : <>Import {fileTotalRows} rows</>}
                </Button>
              </>
            )}
            {importStep === "result" && (
              <Button onClick={() => { setImportDialogOpen(false); resetImportDialog(); }} data-testid="button-smh-horse-import-close">Close</Button>
            )}
            {importStep === "upload" && (
              <Button variant="outline" onClick={() => setImportDialogOpen(false)} data-testid="button-smh-horse-import-cancel">Cancel</Button>
            )}
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function CustomersPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-customers-title">Customers</h2>
        <Button onClick={() => { resetForm(); setDialogOpen(true); }} data-testid="button-smh-add-customer"><Plus className="h-4 w-4 mr-1" /> Add Customer</Button>
      </div>
      {isLoading ? (
        <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />)}</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {customers.map((c) => (
                <tr key={c.id} className="border-b border-border/50" data-testid={`row-smh-customer-${c.id}`}>
                  ...
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(c)} data-testid={`button-smh-edit-customer-${c.id}`}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(c.id)} data-testid={`button-smh-delete-customer-${c.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Name *</Label><Input value={name} onChange={(e) => setName(e.target.value)} data-testid="input-smh-customer-name" /></div>
            <div className="space-y-1"><Label>Email</Label><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} data-testid="input-smh-customer-email" /></div>
            <div className="space-y-1"><Label>Phone</Label><Input value={phone} onChange={(e) => setPhone(e.target.value)} data-testid="input-smh-customer-phone" /></div>
            <div className="space-y-1"><Label>Remarks</Label><Textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} data-testid="input-smh-customer-remarks" /></div>
            <div className="space-y-1">
              <Label>Status</Label>
              <Select value={status} onValueChange={setStatus}>
                <SelectTrigger data-testid="select-smh-customer-status"><SelectValue /></SelectTrigger>
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => { if (!name.trim()) { toast({ title: "Name required", variant: "destructive" }); return; } mutation.mutate({ name, email: email || null, phone: phone || null, remarks: remarks || null, status }); }} disabled={mutation.isPending} data-testid="button-smh-submit-customer">{mutation.isPending ? "Saving..." : "Save"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function ItemsPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-items-title">Items & Services</h2>
        <div className="flex items-center gap-2">
          <input type="file" ref={fileInputRef} accept=".xlsx,.xls,.csv" className="hidden" onChange={handleFileUpload} data-testid="input-smh-item-import-file" />
          <Button variant="outline" onClick={() => { resetImportDialog(); setImportDialogOpen(true); }} disabled={previewMutation.isPending} data-testid="button-smh-import-items">
            {previewMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Upload className="mr-2 h-4 w-4" />}
            Import Excel
          </Button>
          <Button disabled className="opacity-50 cursor-not-allowed" data-testid="button-smh-add-item-disabled">
      ...
      <div className="flex items-center gap-2 mb-3 p-3 rounded-md bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800">
        ...
        <span className="text-xs text-amber-700 dark:text-amber-400" data-testid="text-smh-netsuite-message">Items are mastered in NetSuite. Use Import to sync items from exported NetSuite data.</span>
      </div>
      <div className="flex items-center gap-2 flex-wrap mb-4">
        <div className="relative"><Search className="absolute left-2 top-2.5 h-3.5 w-3.5 text-muted-foreground" /><Input placeholder="Search items..." value={searchQ} onChange={(e) => setSearchQ(e.target.value)} className="pl-7 h-9 w-[180px]" data-testid="input-smh-search-items" /></div>
        <Select value={filterCategory} onValueChange={setFilterCategory}>
          <SelectTrigger className="w-[140px] h-9" data-testid="select-smh-filter-category"><SelectValue placeholder="All categories" /></SelectTrigger>
        ...
        <Select value={filterActive} onValueChange={setFilterActive}>
          <SelectTrigger className="w-[120px] h-9" data-testid="select-smh-filter-active"><SelectValue placeholder="All" /></SelectTrigger>
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {filtered.map((i) => (
                <tr key={i.id} className="border-b border-border/50" data-testid={`row-smh-item-${i.id}`}>
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          {importStep === "upload" && (
            <div className="space-y-4 py-4">
              <div className="border-2 border-dashed rounded-lg p-8 text-center space-y-3">
                ...
                <Button variant="outline" onClick={() => fileInputRef.current?.click()} disabled={previewMutation.isPending} data-testid="button-smh-select-item-file">
                  {previewMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Reading file...</> : <>Select File</>}
                </Button>
              </div>
            </div>
                ...
                {ITEM_FIELDS.map((field) => (
                  <div key={field.key} className="flex items-center gap-3">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setColumnMapping({ ...columnMapping, [field.key]: v === "__none__" ? "" : v })}
                    >
                      <SelectTrigger data-testid={`select-smh-item-map-${field.key}`} className="w-[220px] overflow-hidden">
                        <SelectValue placeholder="Select column..." className="truncate" />
                      </SelectTrigger>
                      <SelectContent className="max-h-48 max-w-[300px] overflow-y-auto" position="popper" sideOffset={4}>
                        <SelectItem value="__none__">-- Skip --</SelectItem>
          ...
          {importStep === "result" && importResult && (
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-3 gap-2 text-sm">
                <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-green-700 dark:text-green-400" data-testid="text-smh-item-import-imported">{importResult.imported}</p>
                  <p className="text-xs text-muted-foreground">Imported</p>
                </div>
                <div className="text-center p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-yellow-700 dark:text-yellow-400" data-testid="text-smh-item-import-skipped">{importResult.skipped}</p>
                  <p className="text-xs text-muted-foreground">Skipped</p>
                </div>
                <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                  <p className="text-2xl font-bold text-blue-700 dark:text-blue-400" data-testid="text-smh-item-import-total">{importResult.totalRows}</p>
                  <p className="text-xs text-muted-foreground">Total Rows</p>
                </div>
              </div>
          ...
          <DialogFooter>
            {importStep === "mapping" && (
              <>
                <Button variant="outline" onClick={() => setImportStep("upload")} data-testid="button-smh-item-import-back">Back</Button>
                <Button onClick={handleImportSubmit} disabled={!columnMapping.name || importMutation.isPending} data-testid="button-smh-item-start-import">
                  {importMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Importing...</> : <>Import {fileTotalRows} rows</>}
                </Button>
              </>
            )}
            {importStep === "result" && (
              <Button onClick={() => { setImportDialogOpen(false); resetImportDialog(); }} data-testid="button-smh-item-import-close">Close</Button>
            )}
            {importStep === "upload" && (
              <Button variant="outline" onClick={() => setImportDialogOpen(false)} data-testid="button-smh-item-import-cancel">Cancel</Button>
            )}
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function LiveryPackagesPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-packages-title">Livery Packages</h2>
        <Button onClick={() => { resetForm(); setDialogOpen(true); }} data-testid="button-smh-add-package"><Plus className="h-4 w-4 mr-1" /> Add Package</Button>
      </div>
      {isLoading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {packages.map((p) => (
                <tr key={p.id} className="border-b border-border/50" data-testid={`row-smh-package-${p.id}`}>
                  <td className="py-2 px-3 font-medium">{p.name}</td>
                  <td className="py-2 px-3">{formatAED(p.monthlyPrice)}</td>
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => openEdit(p)} data-testid={`button-smh-edit-package-${p.id}`}><Pencil className="h-3.5 w-3.5" /></Button>
                      <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(p.id)} data-testid={`button-smh-delete-package-${p.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div className="space-y-1"><Label>Name *</Label><Input value={name} onChange={(e) => setName(e.target.value)} data-testid="input-smh-package-name" /></div>
            <div className="space-y-1"><Label>Monthly Price (AED)</Label><Input type="number" step="0.01" value={monthlyPrice} onChange={(e) => setMonthlyPrice(e.target.value)} data-testid="input-smh-package-price" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => { if (!name.trim()) { toast({ title: "Name required", variant: "destructive" }); return; } mutation.mutate({ name, monthlyPrice: Math.round(parseFloat(monthlyPrice || "0") * 100) }); }} disabled={mutation.isPending} data-testid="button-smh-submit-package">{mutation.isPending ? "Saving..." : "Save"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function NewAgreementPage({ onNavigate }: { onNavigate: (id: string) => void }) {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4" data-testid="text-smh-new-agreement-title">New Livery Agreement</h2>
      ...
      <div className="flex items-center gap-3 flex-wrap mb-4">
        <Select value={filterStable} onValueChange={setFilterStable}>
          <SelectTrigger className="w-[150px] h-9" data-testid="select-smh-newagr-filter-stable"><SelectValue placeholder="All stables" /></SelectTrigger>
          <SelectContent><SelectItem value="all">All stables</SelectItem>{stables.map((s) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent>
        </Select>
        <div className="flex items-center gap-2">
          <Switch checked={showAvailOnly} onCheckedChange={setShowAvailOnly} data-testid="switch-smh-available-only" />
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {filteredBoxes.map((b) => {
              ...
              return (
                <tr key={b.id} className="border-b border-border/50" data-testid={`row-smh-newagr-box-${b.id}`}>
                  <td className="py-2 px-3 font-medium">{b.name}</td>
                  <td className="py-2 px-3">{stableMap[b.stableId]?.name || "-"}</td>
                  <td className="py-2 px-3"><Badge variant="outline" className="no-default-active-elevate">{b.boxType}</Badge></td>
                  <td className="py-2 px-3">
                  ...
                  <td className="py-2 px-3">
                    {!occupied && b.status === "AVAILABLE" && (
                      <Button size="sm" onClick={() => handleCreate(b)} data-testid={`button-smh-create-agr-${b.id}`}><Plus className="h-3.5 w-3.5 mr-1" /> Create</Button>
                    )}
                  </td>
                </tr>
              );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <Select value={agrType} onValueChange={setAgrType}>
                <SelectTrigger data-testid="select-smh-agreement-type"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1">
              ...
              <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} data-testid="input-smh-agreement-start" />
            </div>
            {agrType === "TEMPORARY" && (
              <div className="space-y-1"><Label>End Date</Label><Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} data-testid="input-smh-agreement-end" /></div>
            )}
            <div className="space-y-1">
              <Label>Customer *</Label>
              <SearchableSelect options={customers.map(c => ({ value: c.id, label: c.name, sublabel: c.email || undefined }))} value={customerId} onValueChange={setCustomerId} placeholder="Select customer" data-testid="select-smh-agreement-customer" />
            </div>
            <div className="space-y-1">
              <Label>Horse (optional)</Label>
              <SearchableSelect options={[{ value: "none", label: "No horse (unassigned)" }, ...horses.map(h => ({ value: h.id, label: h.name }))]} value={horseId} onValueChange={setHorseId} placeholder="Select horse" data-testid="select-smh-agreement-horse" />
            </div>
            <div className="space-y-1">
              <Label>Livery Package</Label>
              <SearchableSelect options={packages.map(p => ({ value: p.id, label: p.name, sublabel: `${formatAED(p.monthlyPrice)}/mo` }))} value={packageId} onValueChange={setPackageId} placeholder="Select package" data-testid="select-smh-agreement-package" />
            </div>
            <div className="space-y-1"><Label>Customer Contact</Label><Input value={customerContact} onChange={(e) => setCustomerContact(e.target.value)} data-testid="input-smh-agreement-contact" /></div>
            <div className="space-y-1"><Label>Ref Number</Label><Input value={refNumber} onChange={(e) => setRefNumber(e.target.value)} data-testid="input-smh-agreement-ref" /></div>
            <div className="col-span-1 md:col-span-2 space-y-1"><Label>Remarks</Label><Textarea value={remarks} onChange={(e) => setRemarks(e.target.value)} data-testid="input-smh-agreement-remarks" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleSubmit} disabled={createMutation.isPending} data-testid="button-smh-submit-agreement">{createMutation.isPending ? "Creating..." : "Create Agreement"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function AgreementsListPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4" data-testid="text-smh-agreements-title">Livery Agreements</h2>
      {isLoading ? (
        <div className="space-y-2">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-10 w-full" />)}</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-xs border-collapse">
            ...
            <tbody>
              {agreements.map((a) => (
                <tr key={a.id} className="border-b border-border/50" data-testid={`row-smh-agreement-${a.id}`}>
                  ...
                  <td className="py-2 px-2">
                    <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(a.id)} data-testid={`button-smh-delete-agr-${a.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                  </td>
                </tr>
              ))}
...
function PostBillingPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-2" data-testid="text-smh-post-billing-title">Post Billing Element</h2>
      ...
      <div className="flex items-center gap-2 flex-wrap mb-4">
        <div className="relative"><Search className="absolute left-2 top-2.5 h-3.5 w-3.5 text-muted-foreground" /><Input placeholder="Search..." value={searchQ} onChange={(e) => setSearchQ(e.target.value)} className="pl-7 h-9 w-[180px]" data-testid="input-smh-search-pb-boxes" /></div>
        <div className="w-[160px]">
          <SearchableSelect options={[{ value: "all", label: "All stables" }, ...stables.map(s => ({ value: s.id, label: s.name }))]} value={filterStable || "all"} onValueChange={(v) => setFilterStable(v === "all" ? "" : v)} placeholder="All stables" data-testid="select-smh-pb-filter-stable" />
        </div>
        <div className="w-[160px]">
          <SearchableSelect options={[{ value: "all", label: "All horses" }, ...uniqueHorses.map(h => ({ value: h.id, label: h.name }))]} value={filterHorse || "all"} onValueChange={(v) => setFilterHorse(v === "all" ? "" : v)} placeholder="All horses" data-testid="select-smh-pb-filter-horse" />
        </div>
        <div className="w-[160px]">
          <SearchableSelect options={[{ value: "all", label: "All customers" }, ...uniqueCustomers.map(c => ({ value: c.id, label: c.name }))]} value={filterCustomer || "all"} onValueChange={(v) => setFilterCustomer(v === "all" ? "" : v)} placeholder="All customers" data-testid="select-smh-pb-filter-customer" />
        </div>
      </div>
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {filteredBoxes.map((b) => {
              const agr = activeAgreementByBoxId[b.id];
              return (
                <tr key={b.id} className="border-b border-border/50" data-testid={`row-smh-pb-box-${b.id}`}>
                  <td className="py-2 px-3 font-medium">{b.name}</td>
                  <td className="py-2 px-3">{stableMap[b.stableId]?.name || "-"}</td>
                  ...
                  <td className="py-2 px-3"><Badge variant="outline" className="no-default-active-elevate">{b.boxType}</Badge></td>
                  <td className="py-2 px-3">
                    <Button size="sm" onClick={() => handleOpenCreate(b)} data-testid={`button-smh-create-be-${b.id}`}><Receipt className="h-3.5 w-3.5 mr-1" /> Create</Button>
                  </td>
                </tr>
              );
            })}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label>Stable</Label>
              <Input value={stableMap[selectedBox?.stableId || ""]?.name || ""} readOnly className="opacity-70" data-testid="input-smh-pb-stable" />
            </div>
            <div className="space-y-1">
              <Label>Box</Label>
              <Input value={selectedBox?.name || ""} readOnly className="opacity-70" data-testid="input-smh-pb-box" />
            </div>
            <div className="space-y-1">
              <Label>Transaction Date *</Label>
              <Input type="date" value={formDate} onChange={(e) => setFormDate(e.target.value)} data-testid="input-smh-pb-date" />
            </div>
            <div className="space-y-1">
              <Label>Ref Number</Label>
              <Input value={formRef} onChange={(e) => setFormRef(e.target.value)} data-testid="input-smh-pb-ref" />
            </div>
            <div className="space-y-1">
              <Label>Horse</Label>
              <SearchableSelect options={horseOptions} value={formHorse} onValueChange={setFormHorse} placeholder="Select horse" data-testid="select-smh-pb-horse" />
            </div>
            <div className="space-y-1">
              <Label>Customer *</Label>
              <SearchableSelect options={customerOptions} value={formCustomer} onValueChange={setFormCustomer} placeholder="Select customer" data-testid="select-smh-pb-customer" />
            </div>
            <div className="space-y-1">
              <Label>Item *</Label>
              <SearchableSelect options={itemOptions} value={formItem} onValueChange={setFormItem} placeholder="Select item" data-testid="select-smh-pb-item" />
            </div>
            <div className="space-y-1">
              <Label>Unit</Label>
              <SearchableSelect options={(selectedItem?.unitOptions || []).map(u => ({ value: u, label: u }))} value={formItemUnit} onValueChange={setFormItemUnit} placeholder="Select unit" data-testid="select-smh-pb-item-unit" />
            </div>
            <div className="space-y-1">
              <Label className="flex items-center gap-2">
                Unit Price
                <button className="text-[10px] text-primary underline" onClick={() => setEditPrice(!editPrice)} data-testid="button-smh-toggle-edit-price">
                  {editPrice ? "Lock" : "Edit"}
                </button>
              </Label>
              <Input type="number" step="0.01" value={formUnitPrice} onChange={(e) => setFormUnitPrice(e.target.value)} readOnly={!editPrice} className={!editPrice ? "opacity-70" : ""} data-testid="input-smh-pb-unit-price" />
            </div>
            <div className="space-y-1">
              <Label>Quantity *</Label>
              <Input type="number" step="0.01" min="0.01" value={formQty} onChange={(e) => setFormQty(e.target.value)} data-testid="input-smh-pb-qty" />
            </div>
            <div className="col-span-2 space-y-1"><Label>Remarks</Label><Textarea value={formRemarks} onChange={(e) => setFormRemarks(e.target.value)} data-testid="input-smh-pb-remarks" /></div>
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleSubmit} disabled={createMutation.isPending} data-testid="button-smh-submit-pb">{createMutation.isPending ? "Saving..." : "Save"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function BillingPage() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4 flex-wrap mb-4">
        <h2 className="text-lg font-semibold" data-testid="text-smh-billing-title">Billing</h2>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            data-testid="button-smh-generate-invoice"
          >
      ...
      <div className="flex items-center gap-2 flex-wrap mb-4">
        <div className="w-[160px]">
          <SearchableSelect options={[{ value: "all", label: "All horses" }, ...horses.map(h => ({ value: h.id, label: h.name }))]} value={bFilterHorse || "all"} onValueChange={(v) => setBFilterHorse(v === "all" ? "" : v)} placeholder="All horses" data-testid="select-smh-billing-filter-horse" />
        </div>
        <div className="w-[160px]">
          <SearchableSelect options={[{ value: "all", label: "All customers" }, ...customers.map(c => ({ value: c.id, label: c.name }))]} value={bFilterCustomer || "all"} onValueChange={(v) => setBFilterCustomer(v === "all" ? "" : v)} placeholder="All customers" data-testid="select-smh-billing-filter-customer" />
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="overflow-x-auto mb-8">
          <table className="w-full text-xs border-collapse">
            <thead><tr className="border-b border-border text-left text-muted-foreground">
              <th className="py-2 px-2"><Checkbox checked={selectedIds.size === filteredLines.length && filteredLines.length > 0} onCheckedChange={toggleAll} data-testid="checkbox-smh-select-all" /></th>
            ...
            <tbody>
              {filteredLines.map((l) => {
                ...
                return (
                  <tr key={l.id} className={`border-b border-border/50 ${l.isVirtual ? "bg-blue-50/50 dark:bg-blue-950/10" : ""}`} data-testid={`row-smh-billing-${l.id}`}>
                    <td className="py-2 px-2"><Checkbox checked={selectedIds.has(l.id)} onCheckedChange={() => toggleSelect(l.id)} data-testid={`checkbox-smh-billing-${l.id}`} /></td>
                    ...
                    <td className="py-2 px-2">
                      {!l.isVirtual && (
                        <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(l.id)} data-testid={`button-smh-delete-be-${l.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                      )}
                    </td>
                  </tr>
                );
      ...
      {invoices.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold mb-3" data-testid="text-smh-invoices-title">Generated Invoices</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead><tr className="border-b border-border text-left text-muted-foreground">
              ...
              <tbody>
                {invoices.map((inv) => (
                  <tr key={inv.id} className="border-b border-border/50" data-testid={`row-smh-invoice-${inv.id}`}>
                    ...
                    <td className="py-2 px-2">
                      <Button variant="ghost" size="icon" onClick={() => deleteInvoiceMutation.mutate(inv.id)} data-testid={`button-smh-delete-invoice-${inv.id}`}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Dialog open={confirmDialog} onOpenChange={setConfirmDialog}>
        <DialogContent>
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button onClick={() => invoiceMutation.mutate()} disabled={invoiceMutation.isPending} data-testid="button-smh-confirm-invoice">{invoiceMutation.isPending ? "Generating..." : "Generate Invoice Draft"}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
function AdminUsersPage() {
  return (
    <Card>
      ...
      <CardContent><p className="text-muted-foreground" data-testid="text-smh-users-placeholder">User management will be configured later.</p></CardContent>
    </Card>
  );
}
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <Button variant="destructive" onClick={() => resetMutation.mutate()} disabled={resetMutation.isPending} data-testid="button-smh-reset-demo-data">
...
export default function StableMasterHubPage() {
  ...
  return (
    <div className="flex h-full" data-testid="page-stable-master-hub">
      <SubSidebar
