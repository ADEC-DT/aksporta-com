...
function UserServicesCell({ userId, enabledServices }: { userId: string; enabledServices: ExternalService[] }) {
  ...
  const { data: allowedSubmodules } = useQuery<AllowedSubmodules>({
  ...
  const updateSubmodulesMutation = useMutation({
    mutationFn: async (submodules: AllowedSubmodules) => {
      return apiRequest("PUT", `/api/admin/users/${userId}/submodules`, { allowedSubmodules: submodules });
    },
    onSuccess: () => {
  ...
  const handleToggleSubmodule = (serviceUrl: string, submoduleKey: string, checked: boolean) => {
    const registryEntry = submoduleRegistry[serviceUrl];
    if (!registryEntry) return;

    const allKeys = registryEntry.map(s => s.key);
    const current = allowedSubmodules ?? {};
    const currentForService = current[serviceUrl] ?? allKeys;

    let newForService: string[];
    if (checked) {
      newForService = currentForService.includes(submoduleKey) ? currentForService : [...currentForService, submoduleKey];
    } else {
      newForService = currentForService.filter(k => k !== submoduleKey);
    }

    const allSelected = allKeys.length === newForService.length && allKeys.every(k => newForService.includes(k));

    const newAllowed = { ...current };
    if (allSelected) {
      delete newAllowed[serviceUrl];
    } else {
      newAllowed[serviceUrl] = newForService;
    }

    const hasAnyRestriction = Object.keys(newAllowed).length > 0;
    updateSubmodulesMutation.mutate(hasAnyRestriction ? newAllowed : null);
  };

  const isSubmoduleChecked = (serviceUrl: string, submoduleKey: string): boolean => {
    if (!allowedSubmodules || !allowedSubmodules[serviceUrl]) return true;
    return allowedSubmodules[serviceUrl].includes(submoduleKey);
  };
          ...
          {isLoading ? (
            ...
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {enabledServices.map(service => {
                ...
                const submodules = submoduleRegistry[serviceUrl];
                return (
                  <div key={service.id}>
                      ...
                      <div className="ml-6 space-y-1 pb-1">
                        {submodules.map(sub => (
                          <div key={sub.key} className="flex items-center gap-2 py-0.5">
                            <Checkbox
                              id={`submodule-${userId}-${serviceUrl}-${sub.key}`}
                              checked={isSubmoduleChecked(serviceUrl, sub.key)}
                              onCheckedChange={(checked) => handleToggleSubmodule(serviceUrl, sub.key, checked as boolean)}
                              disabled={updateSubmodulesMutation.isPending}
                              data-testid={`checkbox-submodule-${userId}-${sub.key}`}
                            />
                            <Label
                              htmlFor={`submodule-${userId}-${serviceUrl}-${sub.key}`}
